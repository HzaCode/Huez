...
def detect_colormap_type(
    ...
    if data_min >= 0 and data_max <= 2 * np.pi and data_range > 5.5:
        if verbose:
            print(
                f"🔵 Detected cyclic data in radians (range: {data_min:.2f} to {data_max:.2f})"
            )
            print("   → Recommended: cyclic colormap")
        return "cyclic"
    ...
    if data_min >= -1.1 and data_max <= 1.1 and crosses_zero:
        if verbose:
            print(
                f"🔴🔵 Detected correlation-like data (range: {data_min:.2f} to {data_max:.2f})"
            )
...
def suggest_colormap(
    data: Union[np.ndarray, list], library: str = "matplotlib", verbose: bool = True
) -> str:
    """
    ...
    recommendations = {
        ...
        "plotly": {"sequential": "Viridis", "diverging": "RdBu", "cyclic": "Phase"},
    }
