...
def palette(
    scheme_name: Optional[str] = None, kind: str = "discrete", n: Optional[int] = None
) -> List[str]:
    """
    ...
    if scheme_name not in _current_config.schemes:
        raise ValueError(
            f"Scheme '{scheme_name}' not found. Available schemes: {list(_current_config.schemes.keys())}"
        )
...
def cmap(scheme_name: Optional[str] = None, kind: str = "sequential") -> str:
    """
...
def export_styles(
    output_dir: str, scheme: Optional[str] = None, formats: Optional[List[str]] = None
) -> None:
    """
...
def preview_gallery(output_dir: str, scheme: Optional[str] = None) -> None:
    """
...
def check_palette(
    scheme: Optional[str] = None, kinds: Optional[List[str]] = None
) -> Dict[str, Any]:
    """
...
def lint_figure(file_path: str, report_path: Optional[str] = None) -> Dict[str, Any]:
    """
...
def check_accessibility(
    ...
    if colors is None:
        if scheme_name is None:
            ...
            if scheme_name is None:
                raise ValueError(
                    "No scheme is active. Call hz.use() first or provide colors/scheme_name."
                )
...
def preview(scheme_name: Optional[str] = None, mode: str = "screen") -> None:
    ...
    if scheme_name is None:
        ...
        if scheme_name is None:
            raise ValueError(
                "No scheme is active. Provide scheme_name or call hz.use() first."
            )
    ...
    for i, color in enumerate(colors):
        rect = plt.Rectangle(
            (i, 0), 1, 1, facecolor=color, edgecolor="black", linewidth=1
        )
        ax1.add_patch(rect)
